...
session_start();
	...
	if($_POST["id"]){
		mysqli_query($mysqli_link_data,"update scclientes set sc_vendedor = '$vnd', visita = '$vis', intervalo='$int', dias='$di', finicio='".(date("Y-m-d H:i:s",strtotime(str_replace("/","-",$fini))))."', sc_editado='".strtotime('now')."' where ".getQVen()." and sc_eliminado=0 and codigo='$id'");
		die('OK');
	}
...
<table width="100%" border="0" cellpadding="10" cellspacing="0">
  <tr>
    ...
    <td width="100"><select style="width:200px;" id="cli_vnd">
        <?
        	$sql2=mysqli_query($mysqli_link,"select * from servicios where cliente = '".$_SESSION['user']['data']['id']."' and servicio = 'tecnoventas'");
			while($row2=mysqli_fetch_array($sql2)){
				?><option <?=$cli["nombre"]==$row2['id']?'selected="selected"':''?> value="<?=$row2['id']?>"><?=$row2['nombre']?></option><?	
			}
		?>
    </select></td>
...
<script type="text/javascript">
	function guardarCli(t){
		if($(t).val!="Guardando..."){
			...
			$.ajax({
				url:'/panel/jcontent/gvisita.php',
				type:'POST',
				data:{
					id:'<?=$cli["codigo"]?>',
					vnd:$('#cli_vnd').val(),
